#!/bin/sh
set -eu
version=${CORTEX_VERSION:-latest}; output=cortex; force=false
...
if [ "$version" = latest ]; then version=$(curl --proto '=https' --tlsv1.2 -fsSL https://api.github.com/repos/crtx-dev/cortex/releases/latest | sed -n 's/.*"tag_name":[[:space:]]*"\([^"]*\)".*/\1/p' | head -1); fi
[ -n "$version" ] || { echo 'Could not resolve the latest Cortex release.' >&2; exit 1; }
if [ -e "$output" ] || [ -L "$output" ]; then $force || { echo "Refusing to overwrite $output; pass --force." >&2; exit 1; }; fi
asset="cortex-${os}-${arch}.tar.gz"; base="https://github.com/crtx-dev/cortex/releases/download/$version"; tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT INT TERM
...
[ "$actual" = "$expected" ] || { echo 'Checksum mismatch.' >&2; exit 1; }; tar -xzf "$tmp/$asset" -C "$tmp" cortex; chmod 0755 "$tmp/cortex"; mv "$tmp/cortex" "$output"
echo "Downloaded Cortex $version to $output"
